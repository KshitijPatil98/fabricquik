...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config config_block.pb -o ${ORDERERNAME}.${NETWORK}.com:${ORDERERPORT} --ordererTLSHostnameOverride ${ORDERERNAME}.${NETWORK}.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
